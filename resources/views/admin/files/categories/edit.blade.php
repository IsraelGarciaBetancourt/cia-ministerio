...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mb-6">
            Editar categoría: <span class="text-blue-600">{{ $category->name }}</span>
        </h1>

        <form action="{{ route('files.groups.categories.update', [$group, $category]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
